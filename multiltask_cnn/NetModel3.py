...
def NetModelLayer(x):
    ...
    # x = _FC('Shared_FC', x, 256, keep_prob)

    # # Smile branch
    # smile_fc1 = _FC('smile_fc1', x, 256, keep_prob)
    # y_smile_conv = _FC('smile_softmax', smile_fc1, 2, keep_prob, 'softmax')

    # # Emotion branch
    # emotion_fc1 = _FC('emotion_fc1', x, 256, keep_prob)
    # y_emotion_conv = _FC('emotion_softmax', emotion_fc1, 7, keep_prob, 'linear')

    # # Gender branch
    # gender_fc1 = _FC('gender_fc1', x, 256, keep_prob)
    # y_gender_conv = _FC('gender_softmax', gender_fc1, 2, keep_prob, 'softmax')

    # return y_smile_conv, y_emotion_conv, y_gender_conv, phase_train, keep_prob

    smile_fc1 = _FC('smile_fc1', x, 256, keep_prob, phase_train)
    smile_fc2 = _FC('smile_fc2', smile_fc1, 256, keep_prob, phase_train)
    y_smile_conv = _FC('smile_softmax', smile_fc2, 2, keep_prob, phase_train, 'softmax')
    # y_smile_conv = _FC('smile_softmax', smile_fc1, 2, keep_prob, 'softmax')

    # Emotion branch
    emotion_fc1 = _FC('emotion_fc1', x, 256, keep_prob, phase_train)
    emotion_fc2 = _FC('emotion_fc2', emotion_fc1, 256, keep_prob, phase_train)
    y_emotion_conv = _FC('emotion_softmax', emotion_fc2, 7, keep_prob, phase_train, 'softmax')
    # y_emotion_conv = _FC('emotion_softmax', x, 7, keep_prob, 'linear')

    # Gender branch
    gender_fc1 = _FC('gender_fc1', x, 256, keep_prob, phase_train)
    gender_fc2 = _FC('gender_fc2', gender_fc1, 256, keep_prob, phase_train)
    y_gender_conv = _FC('gender_softmax', gender_fc2, 2, keep_prob, phase_train, 'softmax')
    # y_gender_conv = _FC('gender_softmax', gender_fc1, 2, keep_prob, 'softmax')

    return y_smile_conv, y_emotion_conv, y_gender_conv, phase_train, keep_prob

def _svm_loss(y_smile_conv, y_emotion_conv, y_gender_conv, y_, mask):
    ...
    smile_hinge_loss = tf.reduce_sum(
        tf.reduce_sum(smile_hinge_loss * smile_hinge_loss, axis=1) * smile_mask) / tf.reduce_sum(
        smile_mask)

    gender_hinge_loss = tf.maximum(y_gender_conv - tf.reduce_sum(y_gender_conv * y_, axis=1, keep_dims=True) + 1.0, 0.0)
    gender_hinge_loss = tf.reduce_sum(
        tf.reduce_sum(gender_hinge_loss * gender_hinge_loss, axis=1) * gender_mask) / tf.reduce_sum(
        gender_mask)

    emotion_hinge_loss = tf.maximum(y_emotion_conv - tf.reduce_sum(y_emotion_conv * y_, axis=1, keep_dims=True) + 1.0,
                                    0.0)
    emotion_hinge_loss = tf.reduce_sum(
        tf.reduce_sum(emotion_hinge_loss * emotion_hinge_loss, axis=1) * emotion_mask) / tf.reduce_sum(
        emotion_mask)
    ...
    total_loss = smile_hinge_loss + emotion_hinge_loss + gender_hinge_loss + l2_loss

    return smile_hinge_loss, emotion_hinge_loss, gender_hinge_loss, l2_loss, total_loss


def _cross_entropy_loss(y_smile_conv, y_emotion_conv, y_gender_conv, y_, mask):
    vector_zero = tf.constant(0., tf.float32, [BATCH_SIZE])
    vector_one = tf.constant(1., tf.float32, [BATCH_SIZE])
    vector_two = tf.constant(2., tf.float32, [BATCH_SIZE])
