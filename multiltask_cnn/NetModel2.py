...
def _svm_loss(y_smile_conv, y_emotion_conv, y_gender_conv, y_, mask):
    ...
    #     emotion_mask)
    hinge_loss = tf.reduce_sum(tf.reduce_sum(hinge_loss, axis=1) * emotion_mask) / tf.reduce_sum(emotion_mask)
...
def train_op(loss, global_step):
    learning_rate = tf.train.exponential_decay(INIT_LR, global_step, DECAY_STEP, DECAY_LR_RATE, staircase=True)*1000
