...
def _input():
	...
	y_ = tf.placeholder(dtype=tf.float32, shape=[None, 9], name='label')
	keep_prob = tf.placeholder(dtype=tf.float32, name='keep_prob')
	weights = tf.placeholder(dtype=tf.float32, name='weights') 
	return x, y_, keep_prob, weights
...
def FC_smile(x, keep_prob, _phase_train):
	with tf.variable_scope('FC1_smile'):
		output = _flattten(x)
		output = batch_norm_fc(output, _phase_train)
		output = _fc(output, 256, 'relu')
	with tf.variable_scope('FC2_smile'):
		output = batch_norm_fc(output, phase_train)
		output = _fc(output, 256, 'relu')
	with tf.variable_scope('linear_smile'):
		output = _fc(output, 2)

	return output

def FC_emotion(x, keep_prob, _phase_train):
	with tf.variable_scope('FC1_emotion'):
		output = _flattten(x)
		output = batch_norm_fc(output, _phase_train)
		output = _fc(output, 1024, 'relu')
	with tf.variable_scope('FC2_emotion'):
		output = batch_norm_fc(output, phase_train)
		output = _fc(output, 1024, 'relu')
	with tf.variable_scope('linear_emotion'):
		output = _fc(output, 7)
