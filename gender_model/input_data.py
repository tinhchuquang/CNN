...
def init_test():
    ...
    for i in range(350000, 400000):
        data_test.append((gender[i], path[i]))
...
def get_train_image(point,batch_szie):
    if point == 0:
        init()
    train_img = []
    i = 0
    while True:
        img = cv2.imread(GENDER_TRAIN_FOLDER+data[i+point][1][3:])
        if not np.isnan(data[i+point][0]):
            train_img.append((img, one_hot((int)(data[i+point][0]), 2)))
        i += 1
        if len(train_img) >= batch_szie:
            break
...
def get_test_image(point, batch_size):
    ...
    i = 0
    while True:
        img = cv2.imread(GENDER_TRAIN_FOLDER+data_test[i+point][1][3:])
