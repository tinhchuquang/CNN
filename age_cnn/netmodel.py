...
def batch_norm_fc(x, phase_train):
	...
	def mean_var_with_update():
		...
		normed = tf.nn.batch_normalization(x, mean, var, beta, gama, 1e-3)
...
def batch_norm(x, n_out, phase_train, scope='bn'):
	...
	with tf.variable_scope(scope):
		...
		def mean_var_with_update():
			ema_apply_op = ema.apply([batch_mean, batch_var])
			with tf.control_dependencies([ema_apply_op]):
				return tf.identity(batch_mean), tf.indentity(batch_var)
...
def NetModelLayer(x):
	...
	y_emtion_conv = _FC('emotion_softmax', emotion_fc2, 7, keep_prob, 'softmax')
	...
	#Age branch 
	age_gc1 = _FC('age_fc1', x, 512, keep_prob)
	age_fc2 = _FC('age_fc2', age_fc1, 512, keep_prob)
	y_age_conv = _FC('age_softmax', age_fc2, 7, keep_prob, 'softmax')

	return y_smile_conv, y_emtion_conv, y_gender_conv, y_age_conv

def _svm_loss(y_smile_conv, y_emtion_conv, y_gender_conv, y_age_conv, y_, mask):
	...
	vector_three = tf.constant(3, tf.float32, [BATCH_SIZE])
	...
	age_mask = tf.cast(tf.equal(mask, vector_three), tf.float32)
	...
	y_age = tf.slice(y_, [0, 0], BATCH_SIZE, 7)
	...
	smile_hinge_loss = tf.maximum(y_smile_conv - tf.reduce_sum(y_smile_conv*y_, axis=1, keep_dims=True)+1.0, 0.0)
	smile_hinge_loss = tf.reduce_sum(tf.reduce_sum(smile_hinge_loss, axis=1)*smile_mask) / tf.reduce_sum(smile_mask)

	gender_hinge_loss = tf.maximum(y_emtion_conv - tf.reduce_sum(y_emtion_conv*y, axis=1, keep_dims=True)+1.0, 0.0)
	gender_hinge_loss = tf.reduce_sum(tf.reduce_sum(gender_hinge_loss, axis=1)*gender_mask) / tf.reduce_sum(gender_mask)

	emotion_hinge_loss = tf.maximum(y_emtion_conv - tf.reduce_sum(y_emotion_conv*y_, axis=1, keep_dims=True)+1.0, 0.0)
	emotion_hinge_loss = tf.reduce_sum(tf.reduce_sum(emotion_hinge_loss, axis=1)* emotion_mask) / tf.reduce_sum(emotion_mask)

	age_hingle_loss = tf.maximum(y_age_conv- tf.reduce_sum(y_age_conv*y_, axis=1)*age_mask) / tf.reduce_sum(age_mask)
	age_hingle_loss = tf.reduce_sum(tf.reduce_sum(y_age_conv*y, axis=1)*age_mask) / tf.reduce_sum(age_mask)
	...
	total_loss = smile_hinge_loss+ emotion_hinge_loss+gender_hinge_loss+age_hingle_loss+l2_loss

	return smile_hinge_loss, emotion_hinge_loss, gender_hinge_loss, l2_loss, total_loss

def _cross_entropy_loss(y_smile_conv, y_emotion_conv, y_gender_conv, y_age_conv, y_, mask):
	vector_zero = tf.constant(0., tf.float32, [BATCH_SIZE])
	vector_one = tf.constant(1., tf.float32, [BATCH_SIZE])
	vector_two = tf.constant(2., tf.float32, [BATCH_SIZE])
	vector_three = tf.constant(3., tf.float32, [BATCH_SIZE])
	...
	smile_cross_entropy = tf.reduce_sum(tf.reduce_sum(-y_smile*tf.log(y_smile_conv), axis=1) * smile_mask) / tf.reduce_sum(smile_mask)
	emotion_cross_entropy = tf.reduce_sum(tf.reduce_sum(-y_smile*tf.log(y_smile_conv), axis=1) * emotion_mask) / tf.reduce_sum(emotion_mask)
	gender_cross_entropy = tf.reduce_sum(tf.reduce_sum(-y_gender*tf.log(y_gender_conv), axis=1) * gender_mask) / tf.reduce_sum(gender_mask)
	age_cross_entropy = tf.reduce_sum(tf.reduce_sum(-y_age*tf.log(y_gender_conv), axis=1)*age_mask) / tf.reduce_sum(age_mask)
	...
	total_loss = smile_cross_entropy +emotion_cross_entropy + gender_cross_entropy+ age_cross_entropy + l2_loss
	return smile_cross_entropy, emotion_cross_entropy, gender_cross_entropy, l2_loss, total_loss

def train_op(loss, global_step):
    learning_rate = tf.train.exponential_decay(INIT_LR, global_step, DECAY_STEP, DECAY_LR_RATE, staircase=False)
